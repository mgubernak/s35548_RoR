...
class ReadersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create reader" do
    assert_difference('Reader.count') do
      post readers_url, params: { reader: { name: @reader.name, number: @reader.number, sname: @reader.sname } }
    end
  ...
  test "should update reader" do
    patch reader_url(@reader), params: { reader: { name: @reader.name, number: @reader.number, sname: @reader.sname } }
